...
MANUFACTURING_OVERHEAD_PER_SET: float = (
    435.0  # 제조경비(잡자재+전력+광열비+폐기물처리비)
)
...
def get_tile_mgmt_quantity(spec_code: str, bath_type: BathType) -> float:
    ...
    try:
        ...
        raise KeyError(
            f"타일관리비 수량(W)이 정의되지 않은 규격입니다: {bath_type=}, {spec_code=}"
        )
...
def compute_cost_for_bathroom(
    ...
    *,
    ...
    # ── ① 면적 / 둘레 계열 ─────────────────────────────
    total_area_m2 = sum(
        (p.width_mm / 1000.0) * (p.height_mm / 1000.0) * p.qty for p in panels
    )
    avg_panel_area_m2 = total_area_m2 / total_panels

    # 프레임 사용량: 판넬 1장당 둘레(손실 2% 포함)
    total_perimeter_m = sum(
        2.0 * ((p.width_mm / 1000.0) + (p.height_mm / 1000.0)) * p.qty for p in panels
    )
    ...
    daily_prod_qty = get_daily_production_qty(
        avg_panel_area_m2
    )  # 타일 일일 생산량 (개/일)
    tiles_per_panel = daily_prod_qty / total_panels  # 판넬당 타일 수량 (개/판넬)
    labor_per_panel = (
        total_labor_cost_per_day / tiles_per_panel
    )  # P_panel (판넬당 생산인건비)
...
def draw_wall_elevation_with_faces(
    ...
    drw.text(
        (margin, 4), f"{wall_label_str} : {width_mm} x {height_mm} mm", fill="black"
    )
    drw.rectangle([x0, y0, x1, y1], outline="black", width=3)
...
def compute_jendai_side_panels(
    shape: str, j_enabled: bool, j_has_step: bool, j_depth: int, j_h: int
):
...
with st.sidebar:
    ...
    # 1. 엑셀파일 업로드 (벽판 sheet)
    wall_cost_file = st.file_uploader(
        "엑셀 업로드 (벽판 sheet 포함 DB)", type=["xlsx", "xls"]
    )
    ...
    if wall_cost_file is not None:
        ...
        try:
            ...
            st.session_state["wall_cost_msg"] = (
                "엑셀 '벽판' 시트에서 원가 파라미터를 읽어 적용했습니다."
            )
        except Exception as ex:
    ...
    frame_label = st.radio(
        "프레임 각수 선택", ["15각", "16각", "19각"], horizontal=True
    )
    ...
    st.subheader("관리비 비율(%)")
    rp = st.number_input(
        "생산관리비율 rₚ (%)", min_value=0.0, max_value=50.0, value=20.0, step=0.5
    )
    rs = st.number_input(
        "영업관리비율 rₛ (%)", min_value=0.0, max_value=50.0, value=20.0, step=0.5
    )
    ...
    if calc:
        ...
        if errors:
            ...
            for i, wid in enumerate([1, 2, 3, 4]):
                ...
                with cols[i % 2]:
                    is_jendai_wall = (
                        ...
                        and j_has_step
                        and (j_wall is not None)
                        and (int(j_wall) == int(wid))
                    )
                    extra = 2 if is_jendai_wall else 0
                    caption = (
                        f"{wall_label('사각형', wid)} (벽면 {len(faces) + extra})개"
                    )
            ...
            if j_enabled and j_has_step and int(j_depth) > 0 and int(j_h) > 0:
                side_rows = compute_jendai_side_panels(
                    "사각형", j_enabled, j_has_step, int(j_depth), int(j_h)
                )
                TH, TW = parse_tile(tile)
                for r in side_rows:
                    r["타일"] = f"{TH} x {TW}"
            ...
            if rows:
                ...
                total_labor = cfg.get(
                    "TOTAL_LABOR_COST_PER_DAY", TOTAL_LABOR_COST_PER_DAY
                )
                ...
                st.write(
                    f"- 조립클립(판넬 1장당): {int(cost_res['accessories_amount']):,} 원"
                )
                st.write(
                    f"- **원재료 소계(판넬 1장당)**: **{cost_res['material_total']:,.0f} 원**"
                )
        ...
        if errors:
            ...
            for i, wid in enumerate([1, 2, 3, 4, 5, 6]):
                ...
                with cols[i % 3]:
                    ...
                    caption = (
                        f"{wall_label('코너형', wid)} (벽면 {len(faces) + extra})개"
                    )
                    st.image(img, caption=caption, use_container_width=False)
            ...
            if j_enabled and j_has_step and int(j_depth) > 0 and int(j_h) > 0:
                side_rows = compute_jendai_side_panels(
                    "코너형", j_enabled, j_has_step, int(j_depth), int(j_h)
                )
                TH, TW = parse_tile(tile)
                for r in side_rows:
                    r["타일"] = f"{TH} x {TW}"
...
st.caption(
    "※ 새 엔진 적용 + 벽판 단가/소계/생산·영업관리비 자동계산 + JSON 내보내기까지 포함."
)
