...
def compute_cost_for_bathroom(
    ...
    *,
    ...
    # ── ② 생산인건비 P_panel ───────────────────────────
    daily_prod_qty = get_daily_production_qty(avg_panel_area_m2)   # 장/일
    labor_per_panel = total_labor_cost_per_day / daily_prod_qty    # P_panel
    sets_per_day = daily_prod_qty / total_panels                   # 세트/일
    labor_per_set = labor_per_panel * total_panels                 # 참고용
        ...
        if errors:
            ...
            if rows:
                ...
                st.write(
                    ...
                    f"세트당 {cost_res['labor_per_set']:,.0f} 원)"
                )
                ...
                st.write(
                    ...
                    f"→ 세트 기준 {int(cost_res['tile_mgmt_cost']):,} 원"
                )

                st.write(
                    f"- 출고 + 렉입고: 판넬 1장당 {int(cost_res['shipping_rack_per_panel']):,} 원 "
                    f"→ 세트 기준 {int(cost_res['shipping_rack_cost']):,} 원"
                )
