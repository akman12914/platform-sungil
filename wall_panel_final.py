...
def layout_300x600(W: int, H: int) -> Tuple[List[Panel], str]:
    ...
    if (W <= 1000) and (H <= 2400):
        ...
        dW = W - 2400
        if dW <= 80:
            panels += [Panel("상부판","",2400-TW,1200),
                       Panel("하부판","",2400-TW,H-1200),
                       Panel("세로판","",W-(2400-TW),H)]
        else:
            panels += [Panel("상부판","",2400,1200),
        ...
        dW = W - 2400
        if dW <= 80:
            panels += [Panel("상부판","u",2400-600,1200), Panel("상부판","d",2400-600,1200)]
            _, vtop, vbot = vertical_balance_round(H, TH)
            panels += [Panel("세로판","u",W-(2400-600),vtop),
                       Panel("세로판","d",W-(2400-600),vbot)]
            newH = H - 2400
            ...
            panels += [Panel("상부판","u",2400,1200), Panel("상부판","d",2400,1200)]
            _, vtop, vbot = vertical_balance_round(H, TH)
            panels += [Panel("세로판","u",W-2400,vtop),
                       Panel("세로판","d",W-2400,vbot)]
            newH = H - 2400
        ...
        panels += [Panel("상부판","l",Lw,1200), Panel("상부판","r",Rw,1200),
                   Panel("하부판","l",Lw,H-1200), Panel("하부판","r",Rw,H-1200)]
        ...
        newH = H - 1200
        _, btop, bbot = vertical_balance_round(newH, TH)
        panels += [Panel("하부판","l,u",Lw,btop), Panel("하부판","r,u",Rw,btop),
                   Panel("하부판","l,d",Lw,bbot), Panel("하부판","r,d",Rw,bbot)]

    elif (3400 < W <= 4800) and (3600 < H <= 4800):
        label = "3400<W<=4800 & 3600<H<=4800 : 좌우 가로발란스 + 상4하4(m1,round)"
        _, Lw, Rw = horizontal_balance_round(W, 600)
        panels += [Panel("상부판","l,u",Lw,1200), Panel("상부판","r,u",Rw,1200),
                   Panel("상부판","l,d",Lw,1200), Panel("상부판","r,d",Rw,1200)]
        newH = H - 2400
        _, btop, bbot = vertical_balance_round(newH, TH)
        panels += [Panel("하부판","l,u",Lw,btop), Panel("하부판","r,u",Rw,btop),
                   Panel("하부판","l,d",Lw,bbot), Panel("하부판","r,d",Rw,bbot)]
...
def layout_250x400(W: int, H: int) -> Tuple[List[Panel], str]:
    ...
    if (W <= 1000) and (H <= 2200):
        ...
        dW = W - 2400
        if dW <= 80:
            panels += [Panel("상부판","",2400-TW,1000),
                       Panel("하부판","",2400-TW,H-1000),
                       Panel("세로판","",W-(2400-TW),H)]
        else:
            panels += [Panel("상부판","",2400,1000),
        ...
        if dW <= 80:
            ...
            _, vtop, vbot = vertical_balance_round(H, TH)
            panels += [Panel("세로판","u",W-(2400-400),vtop),
                       Panel("세로판","d",W-(2400-400),vbot)]
            newH = H - 1000
            _, btop, bbot = vertical_balance_round(newH, TH)
            panels += [Panel("하부판","u",2400-400,btop),
        ...
        dW = W - 2400
        if dW <= 80:
            panels += [Panel("상부판","u",2400-400,1000), Panel("상부판","d",2400-400,1000)]
            _, vtop, vbot = vertical_balance_round(H, TH)
            panels += [Panel("세로판","u",W-(2400-400),vtop),
                       Panel("세로판","d",W-(2400-400),vbot)]
            newH = H - 2000
            ...
            panels += [Panel("상부판","u",2400,1000), Panel("상부판","d",2400,1000)]
            _, vtop, vbot = vertical_balance_round(H, TH)
            panels += [Panel("세로판","u",W-2400,vtop),
                       Panel("세로판","d",W-2400,vbot)]
            newH = H - 2000
        ...
        panels += [Panel("상부판","l",Lw,1000), Panel("상부판","r",Rw,1000),
                   Panel("하부판","l",Lw,H-1000), Panel("하부판","r",Rw,H-1000)]
        ...
        _, Lw, Rw = horizontal_balance_round(W, 400)
        panels += [Panel("상부판","l,u",Lw,1000), Panel("상부판","r,u",Rw,1000),
                   Panel("상부판","l,d",Lw,1000), Panel("상부판","r,d",Rw,1000)]
        newH = H - 2000
        _, btop, bbot = vertical_balance_round(newH, TH)
        panels += [Panel("하부판","l,u",Lw,btop), Panel("하부판","r,u",Rw,btop),
                   Panel("하부판","l,d",Lw,bbot), Panel("하부판","r,d",Rw,bbot)]
...
def build_faces_for_wall(
    ...
    if j_faces:
        ...
        for w in j_faces:
            w = int(w)
            if w <= 0: continue
            faces.append(FaceSpec(wall_id, wl, fi, f"{wl}F{fi}", acc, acc+w, 0, bh, w, bh, "jendai-lower"))
            acc += w
            fi += 1
        uh = max(0, height_mm - bh)
        if uh > 0:
            faces.append(FaceSpec(wall_id, wl, fi, f"{wl}F{fi}", 0, width_mm, bh, bh+uh, width_mm, uh, "jendai-upper"))
        return faces

    # C) 분할 없음
    faces.append(FaceSpec(wall_id, wl, 1, f"{wl}F1", 0, width_mm, 0, height_mm, width_mm, height_mm, "single"))
    return faces
...
def collect_all_faces(
    ...
    for wid in wall_ids:
        ...
        jf = [int(v) for v in (j_faces or [])] if (j_wall is not None and int(j_wall)==wid and j_faces) else None
        faces = build_faces_for_wall(
            shape=shape, wall_id=wid, width_mm=Wk, height_mm=int(H_eff),
            door_tuple=door_tuple, j_faces=jf, band_h=1000
        )
    ...
    for f in faces:
        try:
            ...
            for i, p in enumerate(panels, 1):
                rows.append({
                    "벽": f.wall_label, "벽면": f.face_label, "분할사유": f.note,
                    "타일": f"{TH}×{TW}", "규칙분기": branch,
                    "panel_no": i, "kind": p.kind, "pos": p.pos,
                    "width_mm": p.w, "height_mm": p.h,
                    "face_w": f.width_mm, "face_h": f.height_mm,
                })
        except Exception as ex:
            errs.append({
                "벽": f.wall_label, "벽면": f.face_label, "face_w": f.width_mm, "face_h": f.height_mm,
                "타일": f"{TH}×{TW}", "error": str(ex), "분할사유": f.note
            })
    return rows, errs
...
if shape == "사각형":
    ...
    if has_jendai:
        ...
        if j_step == "있음":
            j1 = st.number_input("젠다이 면1 폭 (mm)", min_value=0, value=int(j_W//3), step=10)
            j2 = st.number_input("젠다이 면2 폭 (mm)", min_value=0, value=int(j_W//3), step=10)
            j3 = st.number_input("젠다이 면3 폭 (mm)", min_value=0, value=int(j_W - j1 - j2), step=10)
            if (j1 + j2 + j3) != j_W:
                errors.append(f"젠다이 면 폭 합(={j1+j2+j3})이 젠다이 벽폭(={j_W})과 다릅니다.")
            j_faces = [j1, j2, j3]
        else:
            j1 = st.number_input("젠다이 면1 폭 (mm)", min_value=0, value=int(j_W), step=10)
            if j1 != j_W:
                errors.append(f"젠다이 면 폭(={j1})은 젠다이 벽폭(={j_W})과 같아야 합니다.")
            j_faces = [j1]
    ...
    if calc:
        ...
        if errors:
            ...
            for i, wid in enumerate([1,2,3,4]):
                ...
                jf = j_faces if (has_jendai and 'j_wall' in locals() and int(j_wall)==wid) else None

                faces = build_faces_for_wall("사각형", wid, Wk, int(H_eff), door_tuple, jf, 1000)
                all_faces.extend(faces)
                img = draw_wall_elevation_with_faces(wall_label("사각형", wid), Wk, int(H_eff), faces, target_h_px=280)
                with cols[i%2]:
            ...
            if rows:
                ...
                order = (df.groupby(["kind", "벽판폭", "벽판높이"], as_index=False)
    ...
    j_faces = []
    if has_jendai:
        j_wall = st.number_input("젠다이 벽 번호", min_value=1, max_value=6, value=2, step=1)
        j_W = corner_wall_width_of(int(j_wall), W)
        j_step = st.radio("젠다이 단차", ["없음", "있음"], horizontal=True, key="corner_j_step")
        if j_step == "있음":
            cj1 = st.number_input("젠다이 면1 폭 (mm)", min_value=0, value=int(j_W//2), step=10, key="cj1")
            cj2 = st.number_input("젠다이 면2 폭 (mm)", min_value=0, value=int(j_W - cj1), step=10, key="cj2")
            if (cj1 + cj2) != j_W:
                errors.append(f"젠다이 면 폭 합(={cj1+cj2})이 젠다이 벽폭(={j_W})과 다릅니다.")
            j_faces = [cj1, cj2]
        else:
            cj1 = st.number_input("젠다이 면1 폭 (mm)", min_value=0, value=int(j_W), step=10, key="cj_only")
            if cj1 != j_W:
                errors.append(f"젠다이 면 폭(={cj1})은 젠다이 벽폭(={j_W})과 같아야 합니다.")
            j_faces = [cj1]
    ...
    if calc:
        ...
        if errors:
            ...
            preview_img = draw_corner_preview(W=W, has_split=(split_kind=="구분 있음"), canvas_w=240)
            st.image(preview_img, caption="코너형 도면(평면) 미리보기", width=max(160, int(preview_img.width/3)), use_container_width=False)
            ...
            for i, wid in enumerate([1,2,3,4,5,6]):
                ...
                jf = j_faces if (has_jendai and 'j_wall' in locals() and int(j_wall)==wid) else None
                faces = build_faces_for_wall("코너형", wid, Wk, int(H_eff), door_tuple, jf, 1000)
                all_faces.extend(faces)
                img = draw_wall_elevation_with_faces(wall_label("코너형", wid), Wk, int(H_eff), faces, target_h_px=280)
                with cols[i%3]:
            ...
            if rows:
                ...
                st.markdown("**동일 치수 벽판 수량 집계**")
                order = (df.groupby(["kind","width_mm","height_mm"], as_index=False)
                           .size().rename(columns={"size":"qty"}))
                order["치수"] = order["width_mm"].astype(int).astype(str) + "×" + order["height_mm"].astype(int).astype(str)
...
st.caption("※ 본 앱은 벽(W1~)과 벽면(W1F#)을 구분해 표기하고, 벽면 단위로 패널(벽판)을 산출·집계합니다. 지원 타일: 300×600, 250×400. 최소 가공치수 80mm 제약 적용.")
